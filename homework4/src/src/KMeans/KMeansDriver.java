...
public class KMeansDriver{
    public static void main(String[] args) throws Exception{
        ...
        String tempOld = tempFolder + "/Old";
        String tempNew = tempFolder + "/New";
        String currentInputCenter = args[1];
        Path in = new Path(args[0]);
        Path out= new Path(tempNew);
        boolean flag = true;
        ...
        while (flag){
            if( repeated != 0 ) {
                currentInputCenter = tempOld;
            }
            Configuration conf = new Configuration();
            conf.set("centerpath", currentInputCenter);  //初始聚类中心文件 TODO
            ...
            if( repeated != 0 ) {
                ...
                flag = !Assistance.isFinished(tempOld, tempNew, Integer.parseInt(args[3]), Float.parseFloat(args[4]));
            }
    ...
    public static void Cluster(String[] args) throws IOException, InterruptedException, ClassNotFoundException{
        Configuration conf = new Configuration();
        if (args.length != 4){
            System.err.println("Usage: <points_folder> <init_center_folder> <out> <k> <threshold>");
            System.exit(2);
        ...
        String tempFolder = args[2] + "/temp";
        String tempNew = tempFolder + "/New";
        conf.set("centerpath", tempNew);  //最后一次聚类中心文件
